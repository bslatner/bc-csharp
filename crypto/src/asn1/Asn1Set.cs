...
//					return ((Asn1OctetString)obj).Parser;
        ...
        protected internal void Sort()
        {
            ...
            Platform.SortArrayWithKeys(keys, items, new DerComparer());
...
#endif
        {
            ...
            private static bool AllZeroesFrom(byte[] bs, int pos)
            {
                while (pos < bs.Length)
                {
                    if (bs[pos++] != 0)
            ...
            {
                int len = System.Math.Min(x.Length, y.Length);
                for (int i = 0; i != len; ++i)
                {
                    byte ai = x[i], bi = y[i];
                    if (ai != bi)
                ...
                if (x.Length > y.Length)
                    return AllZeroesFrom(x, len) ? 0 : 1;
                if (x.Length < y.Length)
                    return AllZeroesFrom(y, len) ? 0 : -1;
                return 0;
            }
        }
    }
